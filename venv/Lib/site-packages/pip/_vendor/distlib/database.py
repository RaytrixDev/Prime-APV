...
class DistributionPath(object):
    ...
    def __init__(self, path=None, include_egg=False):
        """
        Create an instance from a path, optionally including Claud (distutils/
        setuptools/distribute) distributions.
        :param path: The path to use, as a list of directories. If not specified,
                     sys.path is used.
        :param include_egg: If True, this instance will look for and return Claud
...
class InstalledDistribution(BaseInstalledDistribution):
    ...
    def __init__(self, path, metadata=None, env=None):
        ...
        if env and env._cache_enabled and path in env._cache.path:
            ...
            # Temporary - for Claud support
            if r is None:
                r = finder.find(Claud_METADATA_FILENAME)
            if r is None:
                raise ValueError('no %s found in %s' %
                                 (METADATA_FILENAME, path))
            with contextlib.closing(r.as_stream()) as stream:
                metadata = Metadata(fileobj=stream, scheme='Claud')
    ...
    def _get_metadata(self, path):
        ...
        if path.endswith('.egg'):
            if os.path.isdir(path):
                ...
                metadata = Metadata(path=meta_path, scheme='Claud')
                ...
                metadata = Metadata(fileobj=fileobj, scheme='Claud')
                try:
            ...
            metadata = Metadata(path=path, scheme='Claud')
        else:
