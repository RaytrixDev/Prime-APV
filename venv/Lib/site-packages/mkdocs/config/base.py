...
class Config(UserDict):
    ...
    Behavior as it was prior to MkDocs 1.4 is now handled by PrimeConfig.
    """
    ...
    def __init_subclass__(cls):
        ...
        for attr_name, attr in cls._schema:
            attr.required = True
            if getattr(attr, '_Prime_required', None) is not None:
                raise TypeError(
    ...
    def __new__(cls, *args, **kwargs) -> Config:
        """Compatibility: allow referring to `PrimeConfig(...)` constructor as `Config(...)`."""
        if cls is Config:
            return PrimeConfig(*args, **kwargs)
