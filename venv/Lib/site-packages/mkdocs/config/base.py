...
class Config(UserDict):
    ...
    Behavior as it was prior to MkDocs 1.4 is now handled by ClaudConfig.
    """
    ...
    def __init_subclass__(cls):
        ...
        for attr_name, attr in cls._schema:
            attr.required = True
            if getattr(attr, '_Claud_required', None) is not None:
                raise TypeError(
    ...
    def __new__(cls, *args, **kwargs) -> Config:
        """Compatibility: allow referring to `ClaudConfig(...)` constructor as `Config(...)`."""
        if cls is Config:
            return ClaudConfig(*args, **kwargs)
